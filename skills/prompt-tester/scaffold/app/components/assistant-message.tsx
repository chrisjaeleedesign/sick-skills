...
import { BranchIcon, MessageActionButton, RerunIcon } from './message-action-button'
...
export function AssistantMessage({ message, streaming, streamingText, streamingImages, onRerun, onBranch }: Props) {
      ...
      <MessageMeta
        actions={
          !streaming ? (
            <>
              {onRerun ? (
                <MessageActionButton onClick={onRerun} ariaLabel="Rerun this response">
                  <RerunIcon />
                </MessageActionButton>
              ) : null}
              {onBranch ? (
