...
You are updating the studio app code in `.design/studio/` to the latest version from the skill scaffold, while preserving user data.

## Steps

1. **Verify `.design/` exists** — if not, tell the user to run `/design-studio` first to initialize.
...
3. **Preserve user data** — do NOT overwrite:
   ...
   - `.design/journal.db`
   - `.design/journal.jsonl`
   - `.design/references/` (all screenshot files)
   - `.design/studio/app/prototypes/` (all prototype subdirectories — do NOT touch any family/version folders, but DO update `studio/app/prototypes/layout.tsx`)
   - `.design/studio/app/lib/seed-data.ts` (if exists)
...
5. **Check for running server:** Run `lsof -i :<port>` using the port from `.design/manifest.json` settings. If the server is running, warn the user: "The studio server is still running — restart it to pick up the updates."
...
- If `.design/journal.jsonl` exists but `.design/journal.db` doesn't, the migration will happen automatically when the server starts (handled by `db.ts`).
