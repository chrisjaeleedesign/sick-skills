"use client";

import { useState, useEffect, useCallback, useRef } from "react";
import { X, Plus, Clock, Link2, Trash2 } from "lucide-react";
import type { Entry, Revision, Attachment, EntryRelation, Importance } from "@/app/lib/types";
import { COLOR_PALETTE } from "@/app/lib/types";
import { IMPORTANCE_OPTIONS } from "@/app/lib/constants";
import { TagPill, FamilyBadge, KindBadge, ImportanceBadge } from "@/app/components/badges";

// ---------------------------------------------------------------------------
// Sub-components
// ---------------------------------------------------------------------------

interface EntryDetailData extends Entry {
  revisions: Revision[];
  attachments: Attachment[];
  relations: EntryRelation[];
}

function isImageAttachment(type: string): boolean {
  return type === "image" || type === "screenshot";
}

function RevisionForm({ onSubmit, onCancel }: { onSubmit: (body: string) => void; onCancel: () => void }) {
  const [body, setBody] = useState("");
  const ref = useRef<HTMLTextAreaElement>(null);
  useEffect(() => { ref.current?.focus(); }, []);

  return (
    <div className="mb-4 rounded-xl border border-border bg-card p-3">
      <textarea
        ref={ref}
        value={body}
        onChange={(e) => {
          setBody(e.target.value);
          e.target.style.height = "auto";
          e.target.style.height = e.target.scrollHeight + "px";
        }}
        placeholder="New revision..."
        className="mb-2 w-full resize-none rounded-lg border border-border bg-surface-1 px-3 py-2 text-[13px] leading-6 text-text-primary outline-none focus:border-surface-3"
        rows={3}
      />
      <div className="flex gap-2">
        <button
          disabled={!body.trim()}
          onClick={() => onSubmit(body.trim())}
          className="rounded-lg bg-primary px-3 py-1 text-xs text-primary-foreground hover:bg-primary/90 disabled:opacity-40"
        >
          Save
        </button>
        <button
          onClick={onCancel}
          className="rounded-lg px-3 py-1 text-xs text-text-tertiary hover:bg-surface-1"
        >
          Cancel
        </button>
      </div>
    </div>
  );
}

function ImportanceSelector({ value, onChange }: { value?: string; onChange: (v: string) => void }) {
  return (
    <div className="mt-2 flex items-center gap-1">
      {IMPORTANCE_OPTIONS.map(({ value: opt }) => (
        <button
          key={opt}
          onClick={() => onChange(opt)}
          className={`rounded-full px-2 py-0.5 text-[10px] font-medium transition ${
            value === opt ? "bg-primary text-white" : "text-text-tertiary hover:bg-surface-2"
          }`}
        >
          {opt}
        </button>
      ))}
    </div>
  );
}

// ---------------------------------------------------------------------------
// EntryDetail — inline side panel with revisions, attachments, relations
// ---------------------------------------------------------------------------

export function EntryDetail({
  entryId,
  onClose,
  onUpdate,
  onDelete,
}: {
  entryId: string;
  onClose: () => void;
  onUpdate: () => void;
  onDelete: (id: string) => void;
}) {
  const [data, setData] = useState<EntryDetailData | null>(null);
  const [loading, setLoading] = useState(true);
  const [addingRevision, setAddingRevision] = useState(false);
  const onCloseRef = useRef(onClose);
  onCloseRef.current = onClose;

  const fetchDetail = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/entries?id=${entryId}`);
      if (!res.ok) throw new Error(`${res.status}`);
      setData(await res.json());
    } catch (err) {
      console.error(err);
    }
    setLoading(false);
  }, [entryId]);

  useEffect(() => { fetchDetail(); }, [fetchDetail]);
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => { if (e.key === "Escape") onCloseRef.current(); };
    window.addEventListener("keydown", handleKey);
    return () => window.removeEventListener("keydown", handleKey);
  }, []);

  async function postAction(action: string, payload: Record<string, unknown>) {
    await fetch("/api/entries", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action, ...payload }),
    });
    fetchDetail();
    onUpdate();
  }

  async function handleAddRevision(body: string) {
    await postAction("add-revision", { entry_id: entryId, body, source: "manual" });
    setAddingRevision(false);
  }

  if (loading || !data) {
    return (
      <div className="flex h-full flex-col border-l border-border bg-surface-1">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <span className="text-sm text-text-tertiary">Loading...</span>
          <button onClick={onClose} className="rounded-lg p-2 text-text-tertiary hover:bg-surface-2">
            <X className="h-5 w-5" />
          </button>
        </div>
      </div>
    );
  }

  const colorStyles = data.color ? COLOR_PALETTE[data.color] : null;
  const imageAttachments = data.attachments.filter((a) => isImageAttachment(a.type));

  return (
    <div className="flex h-full flex-col border-l border-border bg-surface-1">
      {/* Header */}
      <div className={`shrink-0 border-b border-border px-6 py-4 ${colorStyles?.bg ?? ""}`}>
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <KindBadge kind={data.kind} />
            {data.importance && <ImportanceBadge importance={data.importance} />}
            {colorStyles && <span className={`inline-block h-2.5 w-2.5 rounded-full ${colorStyles.dot}`} />}
          </div>
          <div className="flex items-center gap-1">
            <button
              onClick={() => { onDelete(entryId); onClose(); }}
              className="rounded-lg p-2 text-text-tertiary hover:bg-red-50 hover:text-red-500"
            >
              <Trash2 className="h-4 w-4" />
            </button>
            <button onClick={onClose} className="rounded-lg p-2 text-text-tertiary hover:bg-surface-2">
              <X className="h-5 w-5" />
            </button>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {data.family && <FamilyBadge family={data.family} />}
          {data.tags.map((tag) => <TagPill key={tag} tag={tag} />)}
        </div>
        <ImportanceSelector
          value={data.importance}
          onChange={(v) => postAction("update-entry", { id: entryId, patch: { importance: v } })}
        />
      </div>

      {/* Scrollable content */}
      <div className="flex-1 overflow-y-auto">
        {/* Image attachments */}
        {imageAttachments.length > 0 && (
          <div className="space-y-2 px-6 py-4">
            {imageAttachments.map((att) => (
              <img
                key={att.id}
                src={`/api/media/${att.path}`}
                alt={att.alt || ""}
                className="w-full rounded-lg"
                loading="lazy"
              />
            ))}
          </div>
        )}

        {/* Revision stack */}
        <div className="px-6 py-4">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="flex items-center gap-1.5 text-xs font-medium text-text-tertiary">
              <Clock className="h-3.5 w-3.5" />
              Revisions ({data.revisions.length})
            </h3>
            <button
              onClick={() => setAddingRevision(!addingRevision)}
              className="flex items-center gap-1 rounded-lg px-2 py-1 text-xs text-text-tertiary hover:bg-surface-2"
            >
              <Plus className="h-3 w-3" />
              Add
            </button>
          </div>

          {addingRevision && (
            <RevisionForm
              onSubmit={handleAddRevision}
              onCancel={() => setAddingRevision(false)}
            />
          )}

          <div className="space-y-3">
            {data.revisions.map((rev, i) => (
              <div
                key={rev.id}
                className={`rounded-xl border px-4 py-3 ${
                  i === 0 ? "border-border bg-card" : "border-surface-2 bg-surface-1/50"
                }`}
              >
                <div className="mb-1 flex items-center gap-2">
                  <span className="text-[10px] font-medium text-text-tertiary">v{rev.seq}</span>
                  <span className="text-[10px] text-text-tertiary">
                    {new Date(rev.created_at).toLocaleDateString("en-US", {
                      month: "short", day: "numeric", hour: "numeric", minute: "2-digit",
                    })}
                  </span>
                  {rev.source !== "manual" && (
                    <span className="rounded-full bg-surface-2 px-1.5 py-0.5 text-[9px] text-text-tertiary">
                      {rev.source}
                    </span>
                  )}
                </div>
                {rev.body && <p className="text-[13px] leading-6 text-text-secondary">{rev.body}</p>}
              </div>
            ))}
          </div>

          {/* Relations */}
          {data.relations.length > 0 && (
            <div className="mt-6">
              <h3 className="mb-2 flex items-center gap-1.5 text-xs font-medium text-text-tertiary">
                <Link2 className="h-3.5 w-3.5" />
                Relations ({data.relations.length})
              </h3>
              <div className="space-y-1">
                {data.relations.map((rel) => (
                  <div
                    key={`${rel.from_id}-${rel.to_id}`}
                    className="flex items-center justify-between rounded-lg bg-surface-2 px-3 py-1.5"
                  >
                    <span className="text-[11px] text-text-secondary">
                      {rel.type} → {rel.from_id === entryId ? rel.to_id : rel.from_id}
                    </span>
                    <button
                      onClick={() => postAction("remove-relation", { from_id: rel.from_id, to_id: rel.to_id })}
                      className="rounded p-0.5 text-text-tertiary hover:text-red-500"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
