...
This is the reference for generating `.agents/wiggum/prompts/review.md`. The create flow writes this to the project's `.agents/wiggum/prompts/review.md`.
...
1. Read `.agents/wiggum/IMPLEMENTATION_PLAN.md` — identify which phases were completed (all tasks `[x]`) to understand scope.
2. Read the spec files in `.agents/wiggum/SPECS/`.
3. Read `.agents/wiggum/AGENTS.md` for project conventions, commands, component library, and code philosophy.
...
1. **UI patterns** (if UI files were touched): Are they using standard patterns (tabs, modals, lists, forms) or inventing novel solutions? Check `.agents/wiggum/AGENTS.md` for the component library — flag hand-rolled components that duplicate library primitives.
2. **Reference alignment**: Check `.agents/wiggum/SPECS/project_spec.md` for a "UI Reference" section. If reference apps are specified, evaluate whether the implementation matches their patterns.
...
3. **Code philosophy**: Check against the Code Philosophy in `.agents/wiggum/AGENTS.md` — flag redundancy, hacky workarounds, unnecessary complexity, premature abstractions.
...
1. Read `.agents/wiggum/IMPLEMENTATION_PLAN.md` to determine which phases were completed
