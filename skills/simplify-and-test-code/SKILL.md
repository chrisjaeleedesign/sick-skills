...
5. Classify complexity before deleting it:
   - **Accidental:** unused abstractions, dead code, duplicated pathways, generated cruft, unnecessary indirection, speculative extensibility, and premature hardening.
   - **Essential:** domain rules, security checks, validation, permissions, retries, logging, persistence compatibility, external API quirks, migrations, concurrency safeguards, and observability that is actually used.
   - **Unknown:** code whose purpose is unclear from current evidence.
6. Remove accidental complexity. Keep essential complexity. Investigate unknown complexity, or leave it in place and report why.
...
Before deleting or flattening code, require concrete evidence that the code is accidental complexity. Use callers, tests, runtime behavior, logs, schemas, fixtures, saved outputs, or user-confirmed intent.

Missing tests are not evidence that behavior is unused.

Do not remove validation, auth, retries, logging, compatibility, error handling, edge cases, migrations, or permission checks unless concrete evidence shows they are unnecessary or harmful.
...
- The code has fewer concepts to understand. Line count is secondary.
