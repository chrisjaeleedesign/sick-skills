...
- **Action**: Edit `SPECS/landing_stream_landing_page.md`.
- **Rule**: Update the documentation to reflect the *new truth*.
- **Constraint**: If the Implementation Plan changes how something works (e.g., "Pinned Header"), the Spec MUST be updated to describe that behavior as the standard. Do not leave the Spec outdated.

## 4. Update Task Tracker
- **Action**: Edit `task.md` (if active).
- **Rule**: Add the new Phase as a top-level task item.
